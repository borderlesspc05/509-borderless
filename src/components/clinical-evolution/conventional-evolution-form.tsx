...
import {
  listConventionalEvolutionDraftsAction,
  loadConventionalEvolutionAction,
  saveConventionalEvolutionAction,
} from "@/app/actions/conventional-evolution-actions";
...
export function ConventionalEvolutionForm({
  patients,
}: ConventionalEvolutionFormProps) {
  const { userName, displayRole, professionalCouncil, hasPermission } =
    useUserRole();

  const canManage = hasPermission(PERMISSIONS.CONVENTIONAL_EVOLUTION_MANAGE);
  ...
  const loadDrafts = useCallback(async () => {
    const result = await listConventionalEvolutionDraftsAction();
  ...
  const loadCurrentRecord = useCallback(async () => {
    ...
    const result = await loadConventionalEvolutionAction(patientId, sessionDate);

    if (!result.success) {
      const message = result.error ?? "Não foi possível carregar o registro.";
  ...
  async function handleSaveDraft() {
    ...
    const result = await saveConventionalEvolutionAction({
    ...
    const successMessage =
      "Rascunho salvo com sigilo. Apenas você tem acesso a este registro.";
  ...
  async function handleGeneratePdf() {
    ...
    if (!contentHtml.trim() || contentHtml === "<br>") {
      const message = "Escreva a evolução antes de gerar o PDF.";
      setFeedback({ type: "error", message });
    ...
    const saveResult = await saveConventionalEvolutionAction({
      patientId: selectedPatient.id,
      patientName: selectedPatient.name,
      sessionDate,
      contentHtml,
    ...
    if (!saveResult.success) {
      const message =
        "PDF gerado com sucesso, mas não foi possível marcar o registro como finalizado.";
  ...
  function resumeDraft(draft: ConventionalEvolutionRecordRow) {
  ...
  return (
    <div className="space-y-6">
      <section className="flex items-start gap-3 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4 text-sm">
        <Lock className="mt-0.5 size-5 shrink-0 text-amber-600" aria-hidden />
        <div className="space-y-1">
          <p className="font-medium text-amber-900 dark:text-amber-100">
            Registro sigiloso — acesso exclusivo do profissional
          </p>
          <p className="text-amber-800/90 dark:text-amber-200/90">
            As evoluções convencionais (Psicologia, Acupuntura, Nutrição,
            Fonoaudiologia, entre outras) ficam restritas a você. Nem
            administradores ou supervisores têm acesso a este conteúdo.
          </p>
        </div>
      </section>
      ...
      <section className="app-surface-card grid gap-4 p-4 lg:grid-cols-3">
        <div className="space-y-2">
          ...
          {activePatients.length === 0 ? (
            <p className="rounded-lg border border-dashed border-border px-3 py-2.5 text-sm text-muted-foreground">
              Nenhum paciente cadastrado. Cadastre um paciente em Aprendizes.
            </p>
          ) : (
            <Select
              value={patientId}
        ...
        <div className="space-y-2">
          <Label htmlFor="conv-evolution-session-date">Data da sessão</Label>
          <Input
            id="conv-evolution-session-date"
            type="date"
            value={sessionDate}
      ...
      {drafts.length > 0 ? (
        <section className="space-y-3">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-semibold">Meus rascunhos recentes</h2>
      ...
      <ProtectedComponent permission={PERMISSIONS.CONVENTIONAL_EVOLUTION_MANAGE}>
